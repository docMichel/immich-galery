...
class GPSManager {
    ...
    openMapModal() {
        ...
        // Centrer sur une position par défaut ou sur les coordonnées existantes
        let centerLat = -22.2711; // Nouméa par défaut
        let centerLng = 166.4416;
        let zoom = 10;
        
        // Si une seule photo sélectionnée avec GPS, centrer dessus
        if (this.selectedPhotos.size === 1) {
            const assetId = Array.from(this.selectedPhotos)[0];
            const photoItem = document.querySelector(`[data-asset-id="${assetId}"]`);
            if (photoItem.classList.contains('has-gps')) {
...
window.confirmMapLocation = function() {
    ...
    if (!manager.selectedMapCoords || manager.selectedPhotos.size === 0) return;
    
    // Copier les coordonnées de la carte dans le presse-papier
    manager.clipboardGPS = {
        latitude: manager.selectedMapCoords.latitude,
        longitude: manager.selectedMapCoords.longitude,
        assetId: 'map-selection',
        thumbSrc: '' // Pas de miniature pour la sélection carte
    };
